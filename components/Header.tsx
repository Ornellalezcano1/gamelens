...
import { Heart, Search, User, Menu, Globe } from 'lucide-react';
...
export function Header({ user }: HeaderProps) {
  ...
  const translations = {
    en: {
      favorites: 'Favorites',
      searchPlaceholder: 'Search games, creators...',
    },
    es: {
      favorites: 'Favoritos',
      searchPlaceholder: 'Buscar juegos, creadores...',
    }
  };
    ...
    <>
      ...
      <header className="sticky top-0 z-40 flex items-center justify-between px-6 py-4 md:px-10 border-b border-white/5 bg-[#131119]/80 backdrop-blur-xl shrink-0 animate-fade-up">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          {/* FAVORITOS: 'hidden sm:flex' para ocultarlo en móvil */}
          <Link 
            href="/favorites" 
            className="hidden sm:flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 bg-pink-600/20 hover:bg-pink-600/40 text-pink-500 rounded-full transition-colors font-semibold border border-pink-600/50"
          >
            <Heart size={18} fill="currentColor" />
            <span>{t.favorites}</span>
          </Link>

          {/* LANGUAGE SWITCH RESTAURADO (Diseño original) */}
          <button
            onClick={toggleLanguage}
            className="flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 bg-white/5 hover:bg-white/10 text-gray-300 rounded-full transition-colors font-semibold border border-white/10 active:scale-95"
